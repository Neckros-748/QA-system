...
from backend.src.docs.parser.docx_parser import DocxParser
...
class DocumentHandler:
	...
	def parse(
		...
		match os.path.splitext(os.path.basename(file_path))[1].strip():
			case ".docx":
				document: DocxDocument = Document(self.file_path)
				self.doc = DocxParser(file_path, document).parse()
			case ".json":
